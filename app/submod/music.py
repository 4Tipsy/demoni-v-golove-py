...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            ...
            bot.lavalink.add_node('127.0.0.1', int(os.environ.get("LAVALINK_PORT")), 'youshallnotpass', 'eu', 'default-node')  # Host, Port, Password, Region, Name
    ...
    async def ensure_voice(self, ctx):
        ...
        if not v_client:
            ...
            if v_client.channel.id != ctx.author.voice.channel.id:
                raise commands.CommandInvokeError('Не тот канал, дурашка')
    ...
    async def disconnect(self, ctx):
        ...
        if not ctx.voice_client:
            ...
            return await ctx.send('Not connected:32xq')

        if not ctx.author.voice or (player.is_connected and ctx.author.voice.channel.id != int(player.channel_id)):
            # Abuse prevention. Users not in voice channels, or not in the same voice channel as the bot
            # may not disconnect the bot.
            return await ctx.send('Не тот войс, менчик')
